...
const SeriesCard: React.FC<SeriesCardProps> = ({ serie }) => {
  ...
  const isFavorited = favoritedContentIds.has(serie.id);

  const handleSelectSerie = () => {
    setSelectedVodContent(serie);
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`View series: ${serie.name}`}
    ...
    >
      ...
      <div className="relative">
        <div className="relative w-full h-48">
          <img
            src={serie.cover || `https://picsum.photos/200/300?random=${serie.id}`}
            alt={serie.name}
            className="w-full h-full object-cover"
            onError={(e) => { e.currentTarget.src = `https://picsum.photos/200/300?random=${serie.id}`; }}
          />
        ...
        <div className="p-3">
          <h4 className="text-base font-bold text-gray-100 truncate" title={serie.name}>{serie.name}</h4>
          <p className="text-sm text-gray-400 mt-1">{serie.genre?.split('|')[0]}</p>
        </div>
        ...
        <button
          ...
          aria-label={isFavorited ? `Unfavorite ${serie.name}` : `Favorite ${serie.name}`}
        >
          <svg
